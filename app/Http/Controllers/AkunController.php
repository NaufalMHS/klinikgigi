...
class AkunController extends Controller
{
    ...
    public function account_submit_user(Request $request)
    {
        $img = $request->foto;
        $imgext = $request->foto->extension();
        $imgname = time() . '-' . User::generateID() . '.' . $imgext;
        ...
        if ($query == true) {
            return redirect('/login')->with('success', 'Berhasil menambahkan data akun.');
        } else {
            return redirect('/login')->with('error', 'Terjadi kesalahan dalam menambahkan data akun.');
        }
    }
